...
export function createtotem(scene, collidableObjects, x, z, totemColor) {
    ...
    totem = new THREE.Mesh(totemGeometry, totemMaterial); // Removido 'const' para usar a variável global
...
export function updateTotem(delta, scene, hitbox, keyType, collidableObjects) {
    ...
    const playerDistance = new THREE.Vector2(
    ...
    if (playerDistance <= 3.5 && hasKey) {
        // Remove a porta da cena e do array de objetos colidíveis
        scene.remove(door);

        //door.user.data.isObstacle = false;
        //door.visible = false;
        ...
        door = null;
        door.geometry.dispose(); // Libera memória da geometria
        door.material.dispose(); // Libera memória do material
        door = null; // Elimina a referência global

        console.log("Porta está no array?", collidableObjects.includes(door));

        // Opcional: remover a chave do inventário
